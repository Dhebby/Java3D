...
public class Facciata extends Group{
	Texture texture = new TextureLoader("../texture/PietraColonna.jpg", TextureLoader.GENERATE_MIPMAP, null).getTexture();
	private static final int N_COL = 6;
	private static final float DIST_COL = 0.15f;
	private static final int N_GRADINI = 3;
	public Facciata(){
		...
		addChild(addStilobate(createApp()));
	...
	/**
	 * Metodo che aggiunge lo stilobate alla scena
	 * @param app Appearance dell'oggetto
	 * @return TransformGroup da aggiungere al BranchGroup
	 */
	public TransformGroup addStilobate(Appearance app){
		TransformGroup transform = new TransformGroup(); //creo oggettp TG
		
		Transform3D t3d = new Transform3D();
		t3d.setTranslation(new Vector3d(0.0, -0.14f, 0.0f));
		Transform3D t3ds = new Transform3D();
		t3ds.setScale(0.6d);
		t3d.mul(t3ds);
		transform.setTransform(t3d);
		
		transform.addChild(new Stilobate(N_GRADINI, (N_COL/9) + (DIST_COL*(N_COL-1)), app));
